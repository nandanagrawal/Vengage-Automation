...
function statusStyle(s: string) {
  if (s === "approved") return { label: "Approved", cls: "text-emerald-700 bg-emerald-50 border-emerald-200", dot: "bg-emerald-400" };
  if (s === "rejected") return { label: "Rejected", cls: "text-red-700 bg-red-50 border-red-200", dot: "bg-red-400" };
  return { label: "Pending", cls: "text-amber-700 bg-amber-50 border-amber-200", dot: "bg-amber-400" };
}
...
function SortIcon({ active, dir }: { active: boolean; dir: SortDir }) {
  return (
    <span className={`ml-1 inline-flex flex-col leading-none ${active ? "text-indigo-600" : "text-gray-300"}`}>
      <span className={`text-[8px] leading-none ${active && dir === "asc" ? "text-indigo-600" : "text-gray-400"}`}>▲</span>
      <span className={`text-[8px] leading-none ${active && dir === "desc" ? "text-indigo-600" : "text-gray-400"}`}>▼</span>
    </span>
  );
}
...
export default function CustomersPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) =>
      ...
      r.centers?.some((c) => c.name.toLowerCase().includes(q))
    );
  }, [rows, search]);
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6 animate-fadeInUp">
        <div>
          ...
          {syncMsg && <p className="text-indigo-600 text-xs mt-2">{syncMsg}</p>}
        </div>
        <div className="flex items-center gap-2 shrink-0 self-end sm:self-auto">
          <button type="button" onClick={() => void onSync()} disabled={syncing}
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border border-gray-200 text-gray-900 text-sm font-semibold hover:bg-gray-100 transition-colors disabled:opacity-50">
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-2xl border border-gray-200 p-6 shadow-2xl" style={{ background: "var(--bg-card)" }}>
            ...
            <h3 className="text-gray-900 font-semibold mb-1">Delete customer?</h3>
            <p className="text-gray-500 text-sm mb-5">
              <span className="text-gray-900 font-medium">{deleteTarget.display_name}</span> will be permanently deleted from the app. This cannot be undone.
            </p>
            <div className="flex justify-end gap-3">
              <button onClick={() => setDeleteTarget(null)} className="px-4 py-2 rounded-xl text-sm text-gray-500 hover:text-gray-900 transition-colors">Cancel</button>
              <button onClick={() => void confirmDelete()} disabled={deleting}
                className="px-4 py-2 rounded-xl text-sm font-semibold bg-red-100 text-red-600 border border-red-200 hover:bg-red-100 transition-colors disabled:opacity-50">
      ...
      <div className="mb-4 animate-fadeInUp">
        <div className="relative max-w-sm">
          ...
          <input type="text" placeholder="Search by name, email or centre…" value={search} onChange={(e) => setSearch(e.target.value)}
      ...
      {/* Table */}
      <div className="rounded-2xl border border-gray-200 overflow-hidden animate-fadeInUp delay-100" style={{ background: "var(--bg-card)" }}>
        {/* Header row */}
        ...
        <div className="divide-y divide-gray-100">
          ...
          {paginated.map((c, i) => {
            ...
            return (
              <div key={c.id}
                className={`grid max-md:grid-cols-1 items-center px-5 py-3.5 hover:bg-gray-50 transition-colors animate-fadeInLeft cursor-pointer ${COLS}`}
                style={{ animationDelay: `${0.02 * i}s` }}
                onClick={() => router.push(`/customers/${c.id}`)}>

                {/* Customer */}
                <div className="flex items-center gap-3 min-w-0 pr-4">
                  <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-indigo-500/20 to-violet-600/20 border border-indigo-200 flex items-center justify-center text-indigo-600 text-xs font-bold shrink-0">
                    {c.display_name[0]?.toUpperCase() ?? "?"}
                  </div>
                  <span className="text-gray-900 text-sm font-medium truncate">{c.display_name}</span>
                </div>
                ...
                <div className="flex justify-center">
                  <span className={`text-xs font-semibold ${c.add_attachment_in_mail ? "text-emerald-700" : "text-gray-400"}`}>
                ...
                <div className="flex items-center justify-center gap-0.5 flex-wrap" onClick={(e) => e.stopPropagation()}>
                  <Link
                    ...
                    className="p-1.5 rounded-lg text-gray-400 hover:text-sky-600 hover:bg-sky-500/10 transition-colors"
                  ...
                  {isAdmin && c.status === "pending" && (
                    <>
                      <button disabled={approving === c.id} onClick={(e) => { e.stopPropagation(); void onApprove(c.id, "approve"); }}
                        className="px-2 py-1 rounded-lg text-xs font-semibold bg-emerald-100 text-emerald-700 border border-emerald-200 hover:bg-emerald-100 transition-colors disabled:opacity-50">
                        {approving === c.id ? "…" : "✓"}
                      </button>
                      <button disabled={approving === c.id} onClick={(e) => { e.stopPropagation(); void onApprove(c.id, "reject"); }}
                        className="px-2 py-1 rounded-lg text-xs font-semibold bg-red-100 text-red-600 border border-red-200 hover:bg-red-100 transition-colors disabled:opacity-50">
                        ✕
                      </button>
                    </>
                  )}
        ...
        {/* Footer */}
        <div className="px-5 py-3 border-t border-gray-200 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <select value={pageSize} onChange={(e) => setPageSize(Number(e.target.value))}
              className="text-xs text-gray-500 bg-gray-50 border border-gray-200 rounded-lg px-2 py-1 focus:outline-none focus:ring-1 focus:ring-indigo-200">
          ...
          {totalPages > 1 && (
            <div className="flex items-center gap-1">
              <button onClick={() => setPage(1)} disabled={page === 1}
                className="px-2 py-1 rounded-lg text-xs text-gray-500 hover:text-gray-900 hover:bg-gray-100 disabled:opacity-30 disabled:cursor-default transition-colors">«</button>
                ...
                .map((p, idx) =>
                  p === "…" ? (
                    ...
                    <button key={p} onClick={() => setPage(p as number)}
                      className={`w-7 h-7 rounded-lg text-xs font-semibold transition-colors ${page === p ? "bg-indigo-600 text-gray-900" : "text-gray-500 hover:text-gray-900 hover:bg-gray-100"}`}>
                      {p}
                    </button>
                  )
                )}
